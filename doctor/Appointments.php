...
// Check if the logged-in user is a psychologist
if ($role === 'Psychologist') {
    // Fetch appointments for the logged-in psychologist
    $stmt = $conn->prepare("SELECT id, user_name, age, mobile, doctor_name, appointment_date 
                             FROM appointment 
                             WHERE doctor_name = ?");
    $stmt->bind_param("s", $username);  // Fetch appointments based on the psychologist's username
    $stmt->execute();
    $result = $stmt->get_result();
